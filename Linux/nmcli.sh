...
# 查看NetworkManager polkit权限
# 这些(用polkit语言描述的)权限由admin配置，不由普通用户变更
# 通常配置文件在/usr/share/polkit-1/actions/org.freedesktop.NetworkManager.policy
# pkaction命令可以显示polkit的描述
# pkaction --action-id org.freedesktop.NetworkManager.network-control --verbose
nmcli general permissions

# 列出NetworkManager日志级别及域
nmcli general logging

# 改变NetworkManager日志级别
...
nmcli dev disconnect em2
   disconnects a connection on interface em2 and marks the device as unavailable for auto-connecting. As a result, no connection will automatically be activated on the device until the device's 'autoconnect' is
...
nmcli dev wifi con "Cafe Hotspot 1" password caffeine name "My cafe"
   creates a new connection named "My cafe" and then connects it to "Cafe Hotspot 1" SSID using password "caffeine". This is mainly useful when connecting to "Cafe Hotspot 1" for the first time. Next time, it is
   better to use nmcli con up id "My cafe" so that the existing connection profile can be used and no additional is created.
